...
public abstract class MineGUIScreen extends Screen {
        ...
        if (debugMode) {
            for (MineGUIWidget widget : widgets) {
                ...
                if (isHovered && button == 0) { // ЛКМ
                    ...
                    setDragging(true);
                    MineGUI.Companion.getLOGGER().info("Начали перетаскивание: {}", widget);
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        if (debugMode && draggingWidget != null) {
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        if (debugMode && draggingWidget != null && button == 0) {
            ...
            MineGUI.Companion.getLOGGER().info("Перетаскиваем виджет: {}", draggingWidget);
